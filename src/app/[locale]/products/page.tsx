...
export default async function ProductsPage() {
  const [products, t] = await Promise.all([fetchProducts(), getTranslations('pages.products')]);
            ...
            <Link key={p.id} href={`products/${p.id}/alter`} className="group block">
              <article className="rounded-lg bg-white shadow transition hover:shadow-lg">
                ...
                <div className="p-4">
                  ...
                  <p className="mt-3 text-blue-600 underline underline-offset-4">{t('alterCta')}</p>
                </div>
              </article>
            </Link>
          ))}
